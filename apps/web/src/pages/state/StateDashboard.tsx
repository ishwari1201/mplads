...
export const StateDashboard: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      let allWorks: StateWork[] = worksData.works || [];

      // Merge local storage recommendations (for dynamic DA -> SA escalation updates)
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-xs">
        <div>
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-900">State Nodal Authority Overview</h1>
              <p className="text-xs text-slate-600 font-medium">Maharashtra State Nodal Monitoring, Risk Concentration & SLA Oversight</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3">
          <Button
            ...
            className="flex items-center space-x-2 text-xs bg-purple-50 hover:bg-purple-100 text-purple-800 border border-purple-200 font-semibold"
          >
            <Bot size={16} />
      ...
      {error && (
        <div className="p-4 rounded-xl bg-rose-50 border border-rose-200 text-rose-700 text-xs flex items-center justify-between">
          <span>{error}</span>
          <button onClick={fetchDashboardData} className="underline hover:text-rose-900 font-semibold">Retry</button>
        </div>
      )}
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-28 rounded-xl animate-pulse bg-slate-200 border border-slate-200" />
          ))}
        </div>
      ) : overview ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card className="p-4 border-slate-200 bg-white shadow-xs">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-xs text-slate-500 font-semibold uppercase">Total State Works</p>
                <h3 className="text-2xl font-bold text-slate-900 mt-1">{overview.total_projects}</h3>
                <p className="text-[11px] text-slate-600 mt-1">Across {overview.active_districts_count} Active Districts</p>
              </div>
              <span className="p-2 bg-sky-50 text-sky-700 border border-sky-200 rounded-lg">
                <Layers size={18} />
              </span>
            </div>
          </Card>
          ...
          <Card className="p-4 border-slate-200 bg-white shadow-xs">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-xs text-slate-500 font-semibold uppercase">Sanctioned Value</p>
                <h3 className="text-2xl font-bold text-emerald-700 mt-1">
                  ₹{(overview.total_sanctioned_amount / 10000000).toFixed(2)} Cr
                </h3>
                <p className="text-[11px] text-slate-600 mt-1">{overview.active_projects} Executing Projects</p>
              </div>
          ...
          <Card className="p-4 border-slate-200 bg-white shadow-xs">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-xs text-slate-500 font-semibold uppercase">High / Critical Risk</p>
                ...
                <p className="text-[11px] text-rose-600 font-medium mt-1">Requires Priority Verification</p>
              </div>
              <span className="p-2 bg-rose-50 text-rose-700 border border-rose-200 rounded-lg">
                <ShieldAlert size={18} />
              </span>
            </div>
          </Card>
          ...
          <Card className="p-4 border-slate-200 bg-white shadow-xs">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-xs text-slate-500 font-semibold uppercase">75-Day SLA Breaches</p>
                <h3 className="text-2xl font-bold text-amber-700 mt-1">{overview.sla_breaches} Delayed</h3>
                <p className="text-[11px] text-amber-800 font-medium mt-1">Statutory Window Expired</p>
              </div>
        ...
        <Card className="p-5 border-slate-200 bg-white shadow-xs">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="p-3 rounded-xl bg-emerald-50 border border-emerald-200 text-center shadow-xs">
              <div className="text-xs text-emerald-800 font-bold">LOW RISK (0–29)</div>
              <div className="text-xl font-bold text-slate-900 mt-1">{overview.risk_distribution.low}</div>
              <div className="text-[10px] text-slate-600">Normal execution</div>
            </div>
            <div className="p-3 rounded-xl bg-sky-50 border border-sky-200 text-center shadow-xs">
              <div className="text-xs text-sky-800 font-bold">MEDIUM RISK (30–59)</div>
              <div className="text-xl font-bold text-slate-900 mt-1">{overview.risk_distribution.medium}</div>
              <div className="text-[10px] text-slate-600">Routine monitoring</div>
            </div>
            <div className="p-3 rounded-xl bg-amber-50 border border-amber-200 text-center shadow-xs">
              <div className="text-xs text-amber-900 font-bold">HIGH RISK (60–79)</div>
              <div className="text-xl font-bold text-slate-900 mt-1">{overview.risk_distribution.high}</div>
              <div className="text-[10px] text-slate-600">District scrutiny</div>
            </div>
            <div className="p-3 rounded-xl bg-rose-50 border border-rose-200 text-center shadow-xs">
              <div className="text-xs text-rose-800 font-bold">CRITICAL RISK (80–100)</div>
              <div className="text-xl font-bold text-slate-900 mt-1">{overview.risk_distribution.critical}</div>
              <div className="text-[10px] text-slate-600">State Nodal inquiry</div>
            </div>
          </div>
        </Card>
      )}
      ...
      <Card className="p-5 border-slate-200 bg-white shadow-xs">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-base font-bold text-slate-900">District Monitoring Directory</h2>
            <p className="text-xs text-slate-600">Execution and risk summary grouped by district collectorate</p>
          </div>
          <Button
            variant="outline"
            ...
            <tbody className="divide-y divide-slate-200 text-slate-700">
              {districts.map((d) => (
                <tr key={d.district_id} className="hover:bg-slate-50/80 transition-colors">
                  <td className="p-3 font-semibold text-slate-900">{d.district_name}</td>
                  <td className="p-3 text-slate-600">{d.collector_name}</td>
                  <td className="p-3 font-medium text-slate-900">{d.total_works}</td>
                  <td className="p-3 text-emerald-700 font-bold">{d.active_works}</td>
                  <td className="p-3">
                    {d.delayed_works > 0 ? (
      ...
      <Card className="p-5 border-slate-200 bg-white shadow-xs">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-base font-bold text-slate-900">State Priority Risk Queue</h2>
            <p className="text-xs text-slate-600">Works flagged with severe payment/progress divergence or ML text/photo anomalies</p>
          </div>
          <Button
            variant="outline"
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs z-50 flex justify-end">
          <div className="w-full max-w-lg bg-white border-l border-slate-200 p-6 flex flex-col h-full overflow-y-auto shadow-2xl">
            ...
            <div className="flex-1 py-4 space-y-4">
              <div className="p-3 rounded-xl bg-purple-50 border border-purple-200 text-xs text-purple-800 font-medium">
            ...
            <form onSubmit={handleAskAssistant} className="pt-4 border-t border-slate-200 flex space-x-2">
              <input
                ...
                className="flex-1 bg-white border border-slate-300 rounded-lg px-3 py-2 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-purple-500 shadow-xs"
              />
              <Button
                type="submit"
                disabled={aiLoading}
