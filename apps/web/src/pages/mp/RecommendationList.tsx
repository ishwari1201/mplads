...
export const RecommendationList: React.FC = () => {
  ...
  const filtered = recommendations.filter((r) =>
    ...
    (r.sector && r.sector.toLowerCase().includes(filterText.toLowerCase()))
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Recommended Works Directory</h2>
          <p className="text-xs text-slate-500">Track Sanction Lifecycle, SLA Deadlines & Financials</p>
        </div>
      ...
      <Card className="bg-white border-slate-200">
          ...
          <CardTitle className="flex items-center justify-between text-slate-900">
            ...
            <div className="relative w-64">
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-300 rounded-lg pl-8 pr-3 py-1.5 text-xs text-slate-900 focus:outline-none focus:border-sky-500"
              />
            </div>
          </CardTitle>
        </CardHeader>
        <CardContent className="pt-4">
          {loading ? (
            ...
            <div className="py-8 text-center text-xs text-slate-400">No project recommendations found.</div>
          ) : (
              ...
              {filtered.map((item: any) => {
                ...
                return (
                  <div
                    ...
                    className="p-4 bg-slate-50 hover:bg-slate-100/80 rounded-xl border border-slate-200/90 hover:border-sky-300 cursor-pointer transition-all flex flex-col md:flex-row md:items-center justify-between gap-4 group shadow-2xs"
                  >
                    <div className="space-y-1.5 flex-1">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-[10px] uppercase tracking-wider font-semibold px-2 py-0.5 rounded bg-white border border-slate-200 text-slate-600 shadow-2xs">
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-[11px] text-slate-500 pt-1">
                        <span className="flex items-center space-x-1">
                          ...
                          <span>{item.address || item.location_address}</span>
                        </span>
                        <span>Sector: {item.sector}</span>
                      </div>
                    </div>

                    <div className="flex flex-row md:flex-col justify-between items-end shrink-0 border-t md:border-t-0 border-slate-200 pt-3 md:pt-0">
                      <div className="text-right">
                        <div className="text-[10px] text-slate-500 uppercase font-medium">Estimated Budget</div>
                      ...
                      <div className="flex items-center space-x-1 text-xs mt-2">
                        <Clock size={13} className={isBreached ? 'text-rose-600' : 'text-amber-600'} />
                        <span className={isBreached ? 'text-rose-700 font-bold' : 'text-slate-600'}>
          ...
          <div className="space-y-4 text-xs text-slate-800">
            ...
            <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 flex items-center justify-between">
              <div className="space-y-1">
                <div className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">Project Identifier</div>
                <div className="font-mono text-sky-700 font-bold">{selectedProject.id}</div>
              </div>
              <div className="flex items-center space-x-2">
                {getStatusBadge(selectedProject.status)}
                <span className="px-2.5 py-1 rounded bg-white border border-slate-200 text-slate-700 font-mono text-[11px] font-bold">
            ...
            <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-2">
              <div className="font-bold text-sky-700 uppercase text-[10px] tracking-wider flex items-center space-x-1.5">
                <DollarSign size={14} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div className="p-2.5 bg-white rounded-lg border border-slate-200">
                  <div className="text-slate-500 text-[10px]">Sanctioned Allocation</div>
                  <div className="text-sm font-extrabold text-emerald-700">
                    ₹{Number(selectedProject.sanctioned_amount || selectedProject.estimated_cost || 0).toLocaleString('en-IN')}
                  </div>
                </div>
                <div className="p-2.5 bg-white rounded-lg border border-slate-200">
                  <div className="text-slate-500 text-[10px]">Disbursed Installment</div>
            ...
            <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-2">
              <div className="font-bold text-purple-700 uppercase text-[10px] tracking-wider flex items-center space-x-1.5">
                <Briefcase size={14} />
                <span>Implementing Agency & Execution Contractor</span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-[11px]">
                <div>
                  <span className="text-slate-500">Nodal Implementing Agency (IA):</span>
                  <div className="font-bold text-slate-800 mt-0.5">
                    {selectedProject.ia_name || 'Maharashtra Public Works Dept (PWD) / Municipal Corp'}
                  </div>
                </div>
                <div>
                  <span className="text-slate-500">Assigned Execution Contractor:</span>
            ...
            <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-2">
              <div className="font-bold text-amber-700 uppercase text-[10px] tracking-wider flex items-center space-x-1.5">
                <Calendar size={14} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-[11px]">
                ...
                <div>
                  <span className="text-slate-500">Statutory SLA Target Days:</span>
                  <div className="font-bold text-amber-800 mt-0.5">
                    {selectedProject.sla_target_days || 75} Days Statutory Window
                  </div>
                </div>
                <div>
                  <span className="text-slate-500">Target Completion Deadline:</span>
            ...
            <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-2 text-[11px]">
              ...
              <div><span className="text-slate-500">Address:</span> <strong className="text-slate-800">{selectedProject.address}</strong></div>
              <div>
              ...
              <div><span className="text-slate-500">Scope:</span> <span className="text-slate-700 italic">"{selectedProject.description}"</span></div>
            </div>
